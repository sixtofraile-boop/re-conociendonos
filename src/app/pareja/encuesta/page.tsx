...
export default function EncuestaPareja() {
  ...
  const handleSiguiente = async () => {
    ...
    if (preguntaActual < PREGUNTAS_PAREJA.length - 1) {
      ...
      setSaving(true);
      // Garantiza que las 23 preguntas estén completas (sin huecos en el array)
      const respFinal = PREGUNTAS_PAREJA.map((p, i) =>
        respCompletas[i] ?? { pregunta_id: p.id, dimension: p.dimension, hist_yo: 5, hist_par: 5, act_yo: 5, act_par: 5 }
      );
      try {
        ...
        <div className="mb-4">
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ background: "#E8ECF0" }}>
            <div
              ...
              style={{ width: `${progreso}%`, background: "#5B8DD9" }}
            />
          </div>
        </div>
